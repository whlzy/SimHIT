...
import argparse
import src.model.GAN.gan as gan
...
class gan_runner(runner.runner):
    ...
    def set_model(self):
        self.generator = gan.Generator(self.config['model']['data_dim'], self.config['model']['hidden_dim'])
        self.discriminator = gan.Discriminator(self.config['model']['data_dim'], self.config['model']['hidden_dim'])
    ...
    def train_one_epoch(self, current_epoch, max_epoch):
        ...
        for i, (batch) in enumerate(self.train_loader):
            ...
            real_label = Variable(torch.ones(batch.shape[0], 1)).cuda()
            fake_label = Variable(torch.zeros(batch.shape[0], 1)).cuda()
            real_out = self.discriminator(real_point)
            discriminator_loss_real = bceloss(real_out, real_label)
            z_discriminator = Variable(torch.randn(batch.shape[0], self.z_dimension)).cuda()
            fake_point = self.generator(z_discriminator)
            fake_out = self.discriminator(fake_point)
            discriminator_loss_fake = bceloss(fake_out, fake_label)
            ...
            output = self.discriminator(fake_point)
            generator_loss = bceloss(output, real_label)
            self.optimizer_generator.zero_grad()
            generator_loss.backward()
            self.optimizer_generator.step()
        
            if (i+1) % self.write_iter == 0:
                self.mmcv_logger.info('Epoch [{}/{}], Generator Loss: {:.4f}'.format(current_epoch + 1, self.max_epoch, generator_loss.item()))
                self.mmcv_logger.info('Epoch [{}/{}], Discriminator Loss: {:.4f}'.format(current_epoch + 1, self.max_epoch, discriminator_loss.item()))
                self.writer.add_scalar('Loss/%s' % ('train_generator'), generator_loss.item(), current_epoch * len(self.train_loader) + i)
                self.writer.add_scalar('Loss/%s' % ('train_discriminator'), discriminator_loss.item(), current_epoch * len(self.train_loader) + i)
