...
import argparse
import src.model.WGAN.wgan as wgan
...
class gan_runner(runner.runner):
    ...
    def set_model(self):
        self.generator = wgan.Generator(self.config['model']['data_dim'], self.config['model']['hidden_dim'])
        self.discriminator = wgan.Discriminator(self.config['model']['data_dim'], self.config['model']['hidden_dim'])
    ...
    def train_one_epoch(self, current_epoch, max_epoch):
        ...
        for i, (batch) in enumerate(self.train_loader):
            ...
            discriminator_loss = fake_out - real_out
